...
function getColIcon(col: Column, colIndex: number): string {
  return (
    col.icon ||
    COLUMN_POSITION_ICONS[colIndex % COLUMN_POSITION_ICONS.length] ||
    "●"
  );
}
...
function renderHeader(
  ...
  const title =
    c(theme.accent, "✦") + " " + chalk.bold.hex(theme.primary)("Kanbee");
  ...
  for (let row = 0; row < cardRows; row++) {
    ...
    for (let ci = 0; ci < numCols; ci++) {
      ...
      if (row < cards.length) {
        ...
        const isSelected =
          ci === state.selectedCol && row === state.selectedCard;
        ...
        if (isSelected) {
          line +=
            " " +
            bg(theme.highlight, theme.bg, padded.substring(0, colWidth - 1));
        } else {
        ...
        line +=
          ...
          empty.padEnd(
            colWidth -
              1 +
              (visLen(empty) === 0 ? 0 : empty.length - visLen(empty)),
          );
        ...
        line = line.substring(
          0,
          line.length -
            empty.padEnd(colWidth - 1 + (empty.length - visLen(empty))).length,
        );
        line += " " + emptyPadded;
      } else {
...
function renderStatusBar(
  ...
  const modeStr = bg(
    theme.primary,
    theme.bg,
    chalk.bold(modeLabels[state.mode] || " NORMAL "),
  );
  ...
  switch (state.mode) {
    case "NORMAL":
      hints = dim(
        "  hjkl: navigate  n: new  ↵: detail  m/M: move  d: del  A: add col  R: rename  X: del col  t: theme",
        theme,
      );
      break;
    case "DETAIL":
...
function renderThemeSelectOverlay(
  state: AppState,
  theme: ThemeColors,
  width: number,
  height: number,
  ...
  const boxWidth = Math.min(50, Math.max(30, width - 10));
  ...
  // Title
  const title = chalk.bold.hex(theme.primary)("Select Theme");
  lines.push(
    border +
      " " +
  ...
  for (let i = 0; i < themes.length; i++) {
    ...
    const isSelected = i === state.themeSelectIndex;
    const isActive = themeKey === state.data.settings.theme;

    let optionText = "";
    if (isSelected) {
      const marker = c(theme.accent, "> ");
      const labelText = chalk.bold.hex(theme.primary)(label);
      optionText = marker + labelText;
    } else {
      const marker = "  ";
      optionText = marker + c(theme.card, label);
    }

    if (isActive) {
      optionText += dim(" ✓", theme);
    }

    const padded = padRight(
      optionText,
      boxWidth - 4 + (optionText.length - visLen(optionText)),
    );
...
function renderDetailOverlay(
  ...
  const titleLabel =
    state.detailField === 0
      ? chalk.bold.hex(theme.primary)("Title")
      : dim("Title", theme);
  lines.push(border + " " + titleLabel + " ".repeat(boxWidth - 9) + border);

  const titleContent =
    state.detailField === 0 && state.editingField
      ? c(theme.card, state.editBuffer) + bg(theme.card, theme.bg, " ")
      : chalk.bold.hex(theme.card)(
          `${icon} ${truncate(card.title, boxWidth - 6)}`,
        );
  lines.push(
    border + " " + padRight(titleContent, boxWidth - 4) + " " + border,
  );
  ...
  const descLabel =
    state.detailField === 1
      ? chalk.bold.hex(theme.primary)("Description")
      : dim("Description", theme);
  lines.push(
    border + " " + descLabel + " ".repeat(Math.max(0, boxWidth - 15)) + border,
  );

  const descContent =
    state.detailField === 1 && state.editingField
      ? c(theme.card, state.editBuffer) + bg(theme.card, theme.bg, " ")
  ...
  const iconLabel =
    state.detailField === 2
      ? chalk.bold.hex(theme.primary)("Icon")
      : dim("Icon", theme);
  const iconValue =
    state.detailField === 2 && state.editingField
      ? c(theme.card, state.editBuffer) + bg(theme.card, theme.bg, " ")
  ...
  lines.push(
    border +
      ...
      ": " +
      iconValue +
      padRight(iconAvailable, Math.max(0, boxWidth - 30 - visLen(iconValue))) +
      " " +
      border,
  );
  ...
  lines.push(
    border +
      ...
      padRight(
        `Column: ${colName}`,
        boxWidth - 4 + (colName.length - visLen(colName)),
      ) +
      " " +
      border,
  );
  ...
  if (card.labels.length > 0) {
    const labelStr = card.labels
      .map((l) => chalk.keyword(l.color)(`● ${l.name}`))
      .join("  ");
    lines.push(
      border +
        " " +
        padRight(
          `Labels: ${labelStr}`,
          boxWidth - 4 + (labelStr.length - visLen(labelStr)),
        ) +
        " " +
        border,
    );
  ...
  lines.push(
    border +
      " " +
      padRight(created, boxWidth - 4 + (created.length - visLen(created))) +
      " " +
      border,
  );
  ...
  if (card.completed) {
    ...
    const done = c(
      theme.secondary,
      `✓ Completed ${doneAt} by ${card.completed.by}`,
    );
    lines.push(
      border +
        " " +
        padRight(done, boxWidth - 4 + (done.length - visLen(done))) +
        " " +
        border,
    );
  }
...
export function render(state: AppState, theme: ThemeColors): void {
  ...
  if (
    state.mode === "DETAIL" ||
    (state.mode === "INSERT" && state.detailField >= 0)
  ) {
  ...
  if (
    state.mode === "PROMPT" ||
    (state.mode === "INSERT" && state.editingField)
  ) {
    showCursor();
  } else {
    hideCursor();
